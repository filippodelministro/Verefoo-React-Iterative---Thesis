...
Formally, let $P_0$ denote the initial set of requirements already enforced in the network and $\{A_1, A_2, A_3, \ldots\}$ the sequence of new NSRs to be introduced. The iterative process unfolds as the following steps:
...
The total number of solver invocations equals the number of new NSRs — a linear cost — but each individual invocation is far cheaper than a single reconfiguration over the full batch, since the solver always operates considering one new requirement only.

\begin{figure}[H]
    \centering
    \hspace*{-0.05\textwidth}\includegraphics[width=1.1\textwidth]{images/iterative-approach.png}
